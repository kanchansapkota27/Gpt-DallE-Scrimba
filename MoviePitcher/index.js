...
async function fetchSynopisis(outline) {
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `
    You are a professional story writer.Write a creative , marketable and engaging story synopsis based on the idea "${outline}". The synopsis should include actors names in brackets after each character. Choose actors that would be ideal for this role. 
    ###
    Idea:A big-headed daredevil fighter pilot goes back to school only to be sent on a deadly mission.
...
async function fetchTitle(synopsis){
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `
    Write a catchy , mysterious and marketable title in 5 words or less for a movie with this synopsis:${synopsis}.
   `,
   temperature:0.7
  });
...
async function fetchImagePrompt(title,synopsis){
  ...
  const response = await openai.createCompletion({
    model: "text-davinci-003",
    prompt: `Write a image prompt for Dall-E2 to create a poster of movie with title and synopsis.The prompt should be as detailed as possible as marketable
    ###
    title: zero Earth
...
async function fetchImageURL(prompt){
  ...
  //if b64_json  <img src="data:image/png"

  setupInputContainer.innerHTML = `<button id="view-pitch-btn" class="view-pitch-btn">View Pitch</button>`
  document.getElementById('view-pitch-btn').addEventListener('click', ()=>{
    document.getElementById('setup-container').style.display = 'none';
    document.getElementById('output-container').style.display = 'flex';
    movieBossText.innerText = `This idea is so good I'm jealous! It's gonna make you rich for sure! Remember, I want 30% 💰`
  })
}
